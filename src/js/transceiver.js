...
export const initPies = ($txrx) => {
  ...
  return (pieCnt) => {
    ...
    for (let i = 0; i < pieCnt; i++) {
      $dividerContainer.appendChild(_.genEl('DIV', {
        classNames: [`divider-${i}`],
      }));
      $hexContainer.appendChild(_.genEl('DIV', {
        classNames: [`hex-${i}`],
...
export const runTransceiver = ($txrx, { serverUrl, commInterval, blinkInterval , blinkCnt}) => {
  ...
  const rotate = () => {
    ...
    return (hex) => {
      const isClock = Math.abs(currHex - hex) < Math.abs(pieCnt - 1 - hex) + currHex;
    ...
    const setTimeoutBlink = () => {
      setTimeout(() => {      
        ...
        if (cnt < blinkCnt)
          ...
          onFulfilled(codes.length ? codes.slice(1) : null);
      }, blinkInterval);
    }
